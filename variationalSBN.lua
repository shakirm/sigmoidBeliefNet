...
--	local b = torch.rand(1, nNodes); -- biases
	...
	for i = 1, maxIter do	
		...
		for n = 1, N do
			...
			KA = torch.cdiv(torch.cmul(negPhi,negMA),MAA); --K0
			
			negMB = torch.Tensor(nNodes,nNodes):fill(1) - MB;
			KB = torch.cdiv(torch.cmul(torch.repeatTensor(phi,nNodes,1),negMB),MBB)
			...
			-- K IS NOT THE SAME AS THE TEST CASE?? WHERE DID IT DIVERGE
			print(mu_n)
			print(torch.sum(K,1))
			os.exit()
			
			-- Update mu_n
			mu_n = sigmoid(b + W*mu_n:t() + W*(mu_n - xi_n):t() + torch.sum(K,1)) -- eq 24, dim (1 x nNode)
			...
			gradW = gradW - torch.cdiv(torch.cmul(gprod,torch.repeatTensor(MA,nNodes,1)),MAA);
			
			gprod = torch.mm(torch.cmul(phi,torch.Tensor(1,nNodes):fill(1) - xi_n):t(),mu_n)
			gradW = gradW + torch.cdiv(torch.cmul(gprod,torch.repeatTensor(MB,nNodes,1)),MBB);
			
			gradW = gradW + torch.mm((xi_n - mu_n):t(),mu_n); -- -- eq 34
			-- Compute lower bound
			_mu = mu_n[{{},{D+1,nNodes}}]
			
			negMu = torch.Tensor(_mu:size()):fill(1);
			KL = torch.sum(torch.cmul(_mu,torch.log(_mu)) + torch.cmul(negMu,torch.log(negMu)))  -- KL term
			
			expect1 = -torch.sum(torch.log(phiA + phiB));
			expect2 = torch.squeeze(torch.mm((mu_n - xi_n),torch.mm(W,mu_n:t())));
